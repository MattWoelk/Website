...
public class Controller {
	public static void main(String[] args) {
		int size = 20; 
		int popSize = 10; //number of sets for each State (MUST BE EVEN)
		...
		//connections = setTestConnections(size);
		...
		iterate(50, curState, connections, popSize);
	...
	public static void iterate(int times, Set[] curState, int[][] connections, int popSize){
		...
		for(int thisMany = 0; thisMany < times; thisMany++){
			...
			for(int i = 0; i < popSize; i++){
				...
				newState[i].empty(curState[0].size());
			}
			newState = getNextState(curState, connections);
	...
	public static Set[] getNextState(Set[] curSet, int[][] connections){
		...
		for(int i = 0; i < (curSet.length - 2)/2; i++){
			...
			//mutate them:
			dummy[0].mutate(2); //PARAMETER
			dummy[1].mutate(2); //PARAMETER
			//insert them:
